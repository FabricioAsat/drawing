...
export const SLine = ({ selectedShape }: { selectedShape: TSelectedShape }) => {
  const [currentSetting, setcurrentSetting] = useState<TLine>();
  const { getLineById, updateLineInfo } = useShapeContext();
  ...
  useEffect(() => {
    ...
    setcurrentSetting(getLineById(selectedShape.id));
  }, [selectedShape]);

  useEffect(() => {
    if (!currentSetting || !selectedShape) return;
    updateLineInfo(currentSetting);
  }, [currentSetting]);
